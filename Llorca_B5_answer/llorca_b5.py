...
ave_size_est = df.groupby('species', as_index=True)['size_est_(cm)'].mean()

output_df = pd.DataFrame({'average_size_est_(cm) ' : ave_size_est})

# output_df = pd.DataFrame({'species' : species,
#                           'average_size_est_(cm) ' : ave_size_est})

output_df.to_csv('b5_output1.csv')